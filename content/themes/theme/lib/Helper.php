...
class Helper {
	static function theme_image( $name, $width = 'auto', $height = 'auto', $alt = '', $echo = true ) {
		self::print_image( self::relative_url( home_url( '/content/themes/theme/dist/images/' . $name ) ), $width, $height, $alt, $echo );
	}

	static function theme_image_url( $name, $echo = true ) {
		$url = self::relative_url( home_url( '/content/themes/theme/dist/images/' . $name ) );
		if ( $echo ) {
			print( $url );
		}
